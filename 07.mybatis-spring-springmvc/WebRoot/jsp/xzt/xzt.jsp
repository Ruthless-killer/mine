...
$(function(){
	$('#xzt_table').datagrid({    
	    url:'${pageContext.request.contextPath}/xzt/showAll.action',    
	    columns:[[        
	    ...
	    toolbar: [{
			...
			handler: function(){
				...
				$.messager.confirm('Confirm', '确认删除?', function(r){
					if (r){
						//alert("删除");
						// exit action;
						if($("#xzt_table").datagrid("getSelections").length!=0){
							var dd = new Array();
							for (i = 0; i < $("#xzt_table").datagrid("getSelections").length; i++) { 
							 ...
							 $.ajax({
					                cache:true,//保留缓存数据
					                type:"POST",//为post请求
					                url:"${pageContext.request.contextPath}/xzt/delet.action",//这是我在后台接受数据的文件名
			...
			text:'修改',
			handler: function(){
				if($("#xzt_table").datagrid("getSelections").length==1){
					//弹出窗口
					$('#xzt_dialog').dialog({    
					    title: '修改信息',    
					    width: 400,    
					    ...
					    onLoad:function(){
					    	...
					    	$("#xztxg_edit_form :text:eq(1)").val($("#xzt_table").datagrid("getSelected").answer);
					    	$("#xztxg_edit_form :text:eq(2)").val($("#xzt_table").datagrid("getSelected").teachername);
					    	$("#xztxg_edit_form :text:eq(3)").val($("#xzt_table").datagrid("getSelected").coursename);
...
<table id="xzt_table" data-options="fit:true"></table> 
<div id="xzt_dialog"></div>  
</body>
</html>